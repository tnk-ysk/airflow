...
PROJECT_ID = "cloud-batch-project"
...
JOB = {
    "task_groups": [{
        "task_spec": {
            "compute_resource": {"cpu_milli": "1000", "memory_mib": "512"},
            "runnables": [{"script": {"text": "echo test"}}],
        }
    }],
}
# BUILD = {
...
class TestCloudBatchHook:
    def setup_method(self):
        with mock.patch(
            "airflow.providers.google.common.hooks.base_google.GoogleBaseHook.__init__",
            new=mock_base_gcp_hook_no_default_project_id,
        ):
            self.hook = CloudBatchHook(gcp_conn_id="test")

    @mock.patch("airflow.providers.google.cloud.hooks.cloud_batch.CloudBatchHook.get_credentials")
    @mock.patch("airflow.providers.google.cloud.hooks.cloud_batch.BatchServiceClient")
    def test_cloud_batch_service_client_creation(self, mock_client, mock_get_creds):
    ...
    @mock.patch("airflow.providers.google.cloud.hooks.cloud_batch.CloudBatchHook.get_conn")
    def test_create_job(self, get_conn):
        get_conn.return_value.create_job.return_value = mock.MagicMock()

        self.hook.create_job(location=LOCATION, job_id=JOB_ID, job=JOB, request_id=REQUEST_ID, project_id=PROJECT_ID)
        mock_operation = get_conn.return_value.create_job

        mock_operation.assert_called_once_with(
            request={"parent": PARENT, "job_id": JOB_ID, "job": JOB, "request_id": REQUEST_ID}, retry=DEFAULT, timeout=None, metadata=()
        )
...
#
#     @mock.patch(
#         "airflow.providers.google.cloud.hooks.cloud_batch.CloudBatchHook._get_build_id_from_operation"
#     )
